--[[
 ...
 Repository at https://github.com/rpmik/Lua-Yawman-Control-Toliss-A339
]]
-- use local to prevent other unknown Lua scripts from overwriting variables (or vice versa)

local scriptedAircraft = "A339"
...
local views = {
	leftSeat = {-0.51816004514694,0.72847199440002,-27.444192886353,360,-11.833331108093},
	rightSeat = {0.57536154985428,0.72847199440002,-27.444192886353,360,-11.833331108093},
	glareshield = {0.031378291547298,0.72847199440002,-27.444192886353,0.56250023841858,-34.33332824707},
	radios = {0.032316524535418,0.53488540649414,-27.600046157837,0.37500038743019,-77.166656494141},
	FMS = {-0.066239587962627,0.47815787792206,-27.767427444458,0.93749964237213,-62.166664123535},
	overhead = {0.052735671401024,0.49626177549362,-27.338132858276,0.1875,66.999969482422},
	EFBor45Left = {-0.82915705442429,0.59305846691132,-27.475925445557,306.75,-38.166641235352},
	pilotThrottles = {-0.51816004514694,0.72847199440002,-27.444192886353,44.812507629395,-45}
}
...
function multipressTolissA339_buttons() 
    -- if aircraft is an A339 then procede
...
-- autopilot control
		...
		if sp1_pressed and last_button(SIXPACK_1) then
			...
			if dpad_up_pressed then
				meterA339Interaction(DPAD_PRESSED,dref_APAirSpeedUp, dref_APAirSpeedUp, 1.0, 2.0) -- at around two seconds, use larger increment
				DPAD_PRESSED = true
			elseif dpad_down_pressed then
				meterA339Interaction(DPAD_PRESSED,dref_APAirSpeedDown, dref_APAirSpeedDown,1.0,2.0)
				DPAD_PRESSED = true
			end
		...
		if sp3_pressed and last_button(SIXPACK_3) then
			...
			if dpad_up_pressed then
				meterA339Interaction(DPAD_PRESSED, dref_APAdjust.AltUp, dref_APAdjust.AltUp, 1.0, 2.0) -- at around two seconds, use larger increment
				DPAD_PRESSED = true
			elseif dpad_down_pressed then
				meterA339Interaction(DPAD_PRESSED, dref_APAdjust.AltDown, dref_APAdjust.AltDown, 1.0, 2.0)
				DPAD_PRESSED = true
			end
			...
			if sp6_pressed and not MULTI_SIXPACK_PRESSED then
				...
				DataRef("LandingLightStatus_Right","sim/cockpit2/switches/landing_lights_on","readonly")
		...
		if sp5_pressed and last_button(SIXPACK_5) then
			...
			if dpad_up_pressed then
				meterA339Interaction(DPAD_PRESSED,dref_APAdjust.HdgUp, dref_APAdjust.HdgUp, 1.0, 3.0) -- at around two seconds, use larger increment
				DPAD_PRESSED = true
			elseif dpad_down_pressed then
				meterA339Interaction(DPAD_PRESSED, dref_APAdjust.HdgDown, dref_APAdjust.HdgDown, 1.0, 3.0)
				DPAD_PRESSED = true
			end
		...
		if sp6_pressed and last_button(SIXPACK_6) then
			...
			if dpad_up_pressed then
				meterA339Interaction(DPAD_PRESSED, dref_APAdjust.VSUp, dref_APAdjust.VSUp, 1.0, 3.0) -- at around two seconds, use larger increment
				DPAD_PRESSED = true
			elseif dpad_down_pressed then
				meterA339Interaction(DPAD_PRESSED, dref_APAdjust.VSDown, dref_APAdjust.VSDown, 1.0, 3.0)
				DPAD_PRESSED = true
			elseif dpad_center_pressed then
				if dref_BaroStdCap == 1 then
					meterA339Interaction(DPAD_PRESSED, dref_APAdjust.Baro1Push, dref_APAdjust.Baro1Push, 1.0, 3.0)
				else
					meterA339Interaction(DPAD_PRESSED, dref_APAdjust.Baro1Pull, dref_APAdjust.Baro1Pull, 1.0, 3.0)
				end
				DPAD_PRESSED = true
			end
...
-- parking brake			
		if left_bumper_pressed and last_button(LEFT_BUMPER) then
			...
			if wheel_up_pressed or wheel_down_pressed then
				meterA339Interaction(BUMPERS_PRESSED, dref_FlightControls.brakesMax, dref_brakesMax, 2.0, 20) -- at around two seconds, use larger increment
...
-- DPAD_up mode
		if dpad_up_pressed and last_button(DPAD_UP) then
			...
			if wheel_up_pressed then
				meterA339Interaction(DPAD_PRESSED, dref_FlightControls.flapsDown, dref_FlightControls.flapsDown, 1.0, 10) -- at around two seconds, use larger increment
				DPAD_PRESSED = true
			elseif wheel_down_pressed then
				meterA339Interaction(DPAD_PRESSED, dref_FlightControls.flapsUp, dref_FlightControls.flapsUp, 1.0, 10) -- at around two seconds, use larger increment
				DPAD_PRESSED = true
			end
...
function meterA339Interaction(boolButtonPressed, strCommandName1, strCommandName2, floatSeconds, floatIntervalSpeed)
...
--set_axis_assignment(RUDDER, "yaw", "normal" )
	...
	do_every_frame("multipressTolissA339_buttons()")
end
